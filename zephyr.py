...
for ix in range(len(L_x)):
	...
	tref=disk.tdisk(mod.mstar, mod.disk_mdot, mod.rstar)
	rtemp=np.linspace(mod.rstar,mod.disk_radmax,num=1000)
	print 'disk tref=',tref

	#f,b=disk.spec_disk (1e14,1e17,geo.mstar,geo.disk_mdot,geo.rstar,geo.disk_radmax)
	spec_freq,disk_spec=disk.spec_disk2 (1e14,1e18,mod.mstar,mod.disk_mdot,mod.rstar,mod.disk_radmax)
	#ax=plt.loglog(f,b)
	...
	if mod.QSO==1:
		...
		for i in range (len(spec_freq)):
			agn_spec[i]=const_agn*spec_freq[i]**mod.alpha_agn
	...
	print "F_2500=",f2500
	print "L_2500=",L_2500,disk.lnu_disk(C/(2500*1e-8),mod.mstar,mod.disk_mdot,mod.rstar,mod.disk_radmax)
	...
	alpha_ox = -0.3838*np.log10(L_2500/L_2kev)
	...
	xref = 1e16
	zref = 1e17
	X_REF_FOR_IP = [xref,0,zref]

	rho_ref = sv.sv_rho(X_REF_FOR_IP)

	nh_ref = RHO2NH * rho_ref

	IP = np.log10(tot_nioniz/((4*PI*(xref*xref+zref*zref)*((10.0**nh_ref))*C)))
	diag.IP.append(IP)

	diag.alpha_ox.append(alpha_ox)



plot(L_x, diag.alpha_ox)
show()
